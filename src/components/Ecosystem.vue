<template>
	<div class="col-span-full grid grid-cols-12 relative text-white mt-20 bg-[#10151c]">
		<div class="col-span-full flex items-center justify-center my-64 relative">
			...
			<img src="@/assets/images/waves.webp" alt="waves" class="absolute top-0 left-1/2 -translate-x-1/2
						pr-96 scale-80">
			<img src="@/assets/images/logo-dolabs-ecosystem.webp" alt="do labs ecosystem" class="absolute
						top-1/2 -translate-y-1/2 pt-36 pr-24">
		</div>
		<container>
			<h2 class="text-5xl text-center mb-48 font-semibold -mt-12">
				...
				<ul class="glide__slides">
					<li v-for="(elem, id) of upcomingProjects" :key="id"
						...
						<a v-if="elem.url !== null && elem.pic != null"
						   class="rounded-[2rem] h-56 w-64 bg-[#141a25]"
						   :href="elem.url" target="_blank" rel="noopener">
							<img :src="elem.pic" :alt="elem.pic" class="bg-transparent">
						</a>
						<div v-else class="rounded-[2rem] h-56 w-64 bg-[#141a25]"></div>
					</li>
				</ul>
			</div>
		</div>
		<container>
			<h3 class="text-2xl text-center font-semibold mb-16 mt-36">
				Melodity major partners
			</h3>
			<div class="grid grid-cols-6 gap-12">
				<div v-for="(elem, id) of partners" :key="id" class="flex flex-col items-center justify-center">
					<div class="bg-white rounded-full h-24 w-24 flex items-center justify-center p-3 mb-4">
						<img :src="elem.pic" :alt="elem.name" class="object-contain">
					</div>
					<h5 class="font-semibold text-center text-xl">
						{{ elem.name }}
					</h5>
				</div>
			</div>
